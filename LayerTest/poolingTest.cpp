...
{{{ 0.33503392 , 2.796556  },{-1.2349964 ,  0.07790577}},{{-0.7392358 , -1.6891558 },{ 0.6077014 ,  0.5915958 }},
  {{-2.4391599 , -0.40140626},{-0.48190227 -0.3232832 }}}});
    ...
    PoolingLayer<Tensor4xf, Device::CPU> p1{std::vector<int> {2,2},std::vector<int> {2,2},max };
    Tensor4xf out1 = p1.forwardImpl( TensorsWithNames<Tensor4xf> { {"x", inputX}});
    TensorsWithNames<Tensor4xf> out2=p1.backwardImpl(Eigen::DSizes<int, 4> {2,3,2,2},out1);
    ...
    PoolingLayer<Tensor4xf, Device::CPU> p2{ std::vector<int> {2,2},std::vector<int> {2,2},avg };
    Tensor4xf out11 = p2.forwardImpl(TensorsWithNames<Tensor4xf> { {"x", inputX}});
    TensorsWithNames<Tensor4xf> out22=p2.backwardImpl(Eigen::DSizes<int, 4> {2,3,2,2},out11);
